...
document.addEventListener('keydown', function(event) {
    ...
    if (event.key === 'r' && blocoSelecionadoParaRotacao) {
        ...
        let rotation = parseInt(blocoSelecionadoParaRotacao.style.transform.replace('rotate(', '').replace('deg)', '')) || 0;
...
function limparTela() {
    ...
    if (blocos.length > 0) {
        ...
        const precoUltimoBloco = parseFloat(ultimoBloco.getAttribute('data-preco'));
